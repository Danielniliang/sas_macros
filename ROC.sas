...
Generating ROC ouput dataset including the output data from logistic regression with multiple combinations of the predictors in the same time
Returning a table including the optimal cut point based on two different criteria (criteria 1: the cut point that has largest sen + spe; criteria 2: the cut point that has largest 2 * sen + spe)
Allowing generating .rtf report generated by proc report
